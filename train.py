...
def show(Sim,Qim,Sm,Qm,Prd):

    Prd = nn.functional.interpolate(Prd, size=(321,321), mode='bilinear',
                                     align_corners=True)  # upsample  # upsample

    _, Prdmask = torch.max(Prd, 1)



    Sid=(255*np.ones((321,10,3))).astype(np.uint8)
    for i in range(64):
        SimC=(255*Sim[i]).permute(1,2,0).cpu().numpy().astype(np.uint8)
        QimC = (255*Qim[i]).permute(1, 2, 0).cpu().numpy().astype(np.uint8)

        SmC = (255*Sm[i].expand(3,321,321)).permute(1,2,0).cpu().numpy().astype(np.uint8)
        QmC = (255*Qm[i].expand(3,321,321)).permute(1,2,0).cpu().numpy().astype(np.uint8)

        PrdmaskC = (255 * Prdmask[i].unsqueeze(0).expand(3,321,321)).permute(1,2,0).cpu().numpy().astype(np.uint8)

        Fim=np.concatenate((SimC, Sid,QimC, Sid,SmC,Sid,QmC,Sid,PrdmaskC), axis=1)

        cv2.imwrite('/home/amin/Desktop/CaNetFarnoosh/ReS/'+str(i)+'.jpg',Fim)
        # cv2.imshow('w',Fim)
        # cv2.waitKey(0)
...
data_dir = '/home/amin/Desktop/VOCdevkit/Aug_VOC2012/VOC2012'
...
turn_off(model)
pat='/home/amin/Desktop/CaNetFarnoosh/checkpoint/fo=1/model/best.pth'
    ...
    with torch.no_grad():
        ...
        for eva_iter in range(options.iter_time):
            ...
            for i_iter, batch in enumerate(valloader):
                ...
                support_rgbORG,query_rgbORG,query_rgb, query_mask, support_rgb, support_mask, history_mask, sample_class, index = batch
                ...
                show(support_rgbORG, query_rgbORG, support_mask, query_mask, pred)
